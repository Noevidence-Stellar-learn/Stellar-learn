...
> Drop finished files into `apps/web/public/assets/...` and list them in
> `ART_MANIFEST` in
> [`packages/game-engine/src/config.ts`](packages/game-engine/src/config.ts)
> so the scenes load them (anything unlisted keeps its placeholder).
> Character, enemy and boss sheets already exist — procedurally generated by
> `npm run sprites:generate`; see
